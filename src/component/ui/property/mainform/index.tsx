...
export default function MainForm({ title }: MainFormProps) {
    ...
    const handleSubmit = async (e: React.ChangeEvent<HTMLFormElement>) => {
        ...
        formData.append('your-subject', "Inquiry about: " + title)
        ...
        const response = await fetch(`${wordpressUrl}/wp-json/contact-form-7/v1/contact-forms/${formId}/feedback`, {
            method: 'POST',
            body: formData,
            headers: {
        ...
        if (result.status === 'mail_sent') {
            setSubmitStatus({ success: true, message: 'Thank you! Your inquiry has been sent.' });
            setFirstName('');
            setLastName('')
            setEmail('')
            setPhoneNumber('')
  ...
  return (
    <div className="wpcf7 js">
      <form onSubmit={handleSubmit} className={style.fields}>
